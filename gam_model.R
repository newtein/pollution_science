...
for (pollutant in pollutants){
  for (lag_t in lag_list){
    ...
    formula_1 = log(cases)~pollutant+s(mean_pressure)+s(mean_rh)+s(mean_wind)+city+date_number+log(cases_shifted)
    formula_2 = log(deaths)~pollutant+s(mean_pressure)+s(mean_rh)+s(mean_wind)+city+date_number+log(deaths_shifted)
    
    gam_y_cases <- gam(formula = formula_1, method = "REML", data=df_cases)
    print("CASES")
    sink(cases_wfname)
    print(summary(gam_y_cases))
    sink()

    gam_y_deaths <- gam(formula = formula_2, method = "REML", data=df_deaths)
    print("DEATHS")
    sink(deaths_wfname)
