...
for (pollutant in pollutants){
  for (lag_t in lag_list){
    ...
    wfname = "D:/CovidPollution/r_files/model_summary_SA1/gam_summary"
    ...
    formula_1 = log(cases)~pollutant+s(mean_pressure)+s(mean_rh)+s(mean_wind)+s(mean_temp)+city+date_number
    formula_2 = log(deaths)~pollutant+s(mean_pressure)+s(mean_rh)+s(mean_wind)+s(mean_temp)+city+date_number
