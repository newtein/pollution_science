...
state_code <- "06"
pollutants <- c("combined")
lag_list <- c("0-7", "0-14", "0-21")
for (pollutant in pollutants){
  for (lag_t in lag_list){
    print(paste(pollutant, lag_t))
    fname = "D:/CovidPollution/r_files/source_file_SA3/R_data"
    wfname = "D:/CovidPollution/r_files/model_summary_SA3/gam_summary"
    ...
    formula_1 = log(cases)~PM2+PM10+O3+s(mean_pressure)+s(mean_rh)+s(mean_wind)+s(mean_temp)+city+date_number+log(cases_shifted)
    formula_2 = log(deaths)~PM2+PM10+O3+s(mean_pressure)+s(mean_rh)+s(mean_wind)+s(mean_temp)+city+date_number+log(deaths_shifted)
